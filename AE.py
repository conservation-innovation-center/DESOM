...
def lstm_autoencoder(input_dim, n_latent = 32, activation = ReLU(max_value = 2.0)):
    """ Build and complie an LSTM autoencoder model in Keras

    Params
    ---
    intpu_dim: list:int
        [T, N] dimensions of input observations
    n_latent: int
    ...
    Return
    ---
    keras.models.Model: autoencoder, encoder, and decoder models
    """
    n_time = input_dim[0]
    n_features = input_dim[-1]
    lstm_input = Input((n_time, n_features), name = 'timeseries_input')
    ...
    encoded, hidden_h, final_c = encoder(lstm_input)

    normalized = BatchNormalization(name = 'batch_norm')(encoded)
    activated = Activation('relu')(normalized)

    # branch 1 - predicting reversed sequence
    # repeated = layers.RepeatVector(n_time)(concatenated) 
    hidden = [hidden_h, final_c] 
    decoder = LSTM(
        units = n_latent,
        activation = None,
        return_sequences = True,
    ...
    decoded = decoder(activated, initial_state = hidden)

    temporal_dense = Dense(n_features, activation = activation, name = 'decoder_0')
    decoded = temporal_dense(decoded)
    
    encoder = Model(inputs = lstm_input, outputs = [encoded, hidden_h, final_c])
    autoencoder = Model(inputs = lstm_input, outputs = decoded)
    decoder = Model(inputs = [encoded, hidden], outputs = decoded)
